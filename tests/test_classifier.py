...
def test_missing_protections_increase_score() -> None:
    ...
    hardened = ProtectionInfo(nx=True, canary=True, relro="full")
    weak = ProtectionInfo(nx=False, canary=False, relro="none")
...
def test_more_call_sites_increase_score() -> None:
    ...
    imports = [DangerousImport("strcpy", "critical", "Overflow", 0x1000)]
    protections = ProtectionInfo(nx=True, canary=True, relro="full")
